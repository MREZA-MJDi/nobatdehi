(() => {
    ...
    const hero = page.querySelector('[data-discover-hero-slider]');
    const heroSlides = hero
        ? [...hero.querySelectorAll('[data-discover-hero-slide]')]
        : [];
    const heroFeature = hero?.querySelector('[data-discover-hero-feature]');
    const heroFeatureName = hero?.querySelector('[data-discover-hero-name]');
    const heroFeatureLocation = hero?.querySelector('[data-discover-hero-location]');
    ...
    const setHeroSlide = (index) => {
        ...
        if (heroFeature && activeSlide) {
            const name = activeSlide.dataset.heroName || 'NOBAT';
            const location = activeSlide.dataset.heroLocation || 'سالن منتخب امروز';
            const url = activeSlide.dataset.heroUrl || '#results';

            heroFeature.href = url;
            heroFeature.setAttribute('aria-label', `مشاهده ${name}`);

            if (heroFeatureName) {
                heroFeatureName.textContent = name;
            }

            if (heroFeatureLocation) {
                heroFeatureLocation.textContent = location;
            }
        }
    };
    ...
    if (heroSlides.length > 1) {
        ...
        if (!prefersReducedMotion) {
            ...
            hero?.addEventListener('mouseleave', () => {
                if (heroTimer || prefersReducedMotion) {
                    return;
                }
